...
class Network:
    ...
    def init(self, weights=None, biases=None):
        ...
        self.weights = weights if weights else [np.random.rand(self.layers[(i + 1)], self.layers[i]) for i in range(len(self.layers) - 1)]
        self.biases = biases if biases else [np.negative(np.random.rand(self.layers[(i + 1)]) * 34) for i in range(len(self.layers) - 1)]
        if not weights:
    ...
    def backprop(self, input, expected_output):
        forward = self.forward(input)
        z = forward[0]
        a = forward[1]
        uw = [0] * self.n_layer
        ub = [0] * self.n_layer
        ua = [0] * self.n_layer
        ua[-1] = expected_output
        ...
        self.weights = np.asarray(self.weights) - np.asarray(uw)
        self.biases = np.asarray(self.biases) - np.asarray(ub)
